"""Shared video transport widget — timeline + tools + transport row.

Promoted from ``mira/ui/edited/video_workshop_bar.py`` (Editor's bar,
the better implementation) to a shared module per spec/130. Used on
BOTH the Picker (surface 07) and the Editor (surface 12) so they show
the same timeline + transport controls — a single widget backs both.

The Picker doesn't wire the marker / snapshot / segment-info extras
(they have no meaning in Pick context); the spec accepts those bits as
"harmless/unused on Picker" and leaves them in the shared widget so the
Editor keeps its full surface unchanged.

Three rows stacked under the canvas when the host's viewport lands on a
video item (spec/56 §1 marker-partition + spec/59 §4-§5 lines):

1. **The marker timeline** (:class:`MarkerTimeline`) — clip bands +
   marker handles + snapshot glyphs + the playhead. (Bare on Picker.)
...
snapshot) — wired by the host (EditorPage / PickerPage), not this
widget.

The whole strip sits inside a **fixed-height reveal host** so the
canvas geometry above is invariant under photo↔video sweeps (the
no-canvas-jump rule, lifted from PickerPage's compact_row Fix A on
2026-06-15). The host's ``setVisible(False)`` hides the inner content
while keeping the reserved space.

The widget is pure presentation: it emits high-level signals, takes
state pushes (set_position / set_playing / set_duration /
set_segment_info / set_volume / etc.), and the host wires those to:
...
* the :class:`~mira.gateway.event_gateway.EventGateway`'s marker /
  segment / snapshot mutators (Editor only)
* the development panel's selection-scoped persistence (Editor only)
"""
...
class VideoWorkshopBar(QWidget):
    ...
    The widget owns no data — the host (EditorPage / PickerPage) loads
    markers / segments / snapshots from the gateway and pushes them in
    via :meth:`set_timeline_model` / :meth:`set_position` / etc.
    """
