...
class EventsPage(QWidget):
    ...
    def _pin_cross_event_dc(self, library_gateway, dc) -> None:
        ...
        ctx = NewRecipeContext(
            event_name="",
            # A cross-event Cut shares the global tag namespace with
            # Collections, so it can't reuse the Collection's own name. Start
            # the name field EMPTY (the placeholder prompts the user) — same
            # as the event-scope New Cut flow. A Cut is a named deliverable;
            # the user gives it its own name rather than inheriting "macro"
            # (always taken) or an auto "macro_2" (meaningless).
            name="",
