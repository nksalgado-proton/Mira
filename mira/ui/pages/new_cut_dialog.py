...
class NewRecipeContext:
    ...
    # spec/114 — overlay control. ``overlay_field_options`` is the
    # vocabulary the dialog's multi-select renders: a list of
    # ``(key, label)`` pairs where ``key`` matches the
    # :data:`core.cut_overlay.OVERLAY_FIELDS` enum
    # ('when'/'where'/'how1'/'how2') and ``label`` is the user-facing
    # string. ``overlay_fields`` is the prefill (the cut's current
    # selection); ``overlay_mode`` is the prefill mode (``None`` =
    # Off | ``'embedded'`` = IPTC | ``'burn_in'`` = pixels). An empty
    # options list disables the multi-select entirely so a host that
    # hasn't wired the vocabulary won't crash; it just hides the
    # control.
    overlay_field_options: List = field(default_factory=list)
...
# spec/162 Round 2d.C — _SaveAsDcNameDialog + _LoadDcDialog retired
# with the dead-code sweep. Both classes were the sub-dialogs the
# retired Save-as-Collection / Load-Collection band-header buttons
# opened; Round 2a's body rebuild orphaned them.
...
class NewCutDialog(QDialog):
    ...
    # spec/162 Round 2d.C — the following builders + handlers retired
    # as dead code after Round 2a's body rebuild:
    #   * _build_recipe_toolbar         (RecipeContainer subsumed it)
    #   * _wrap_as_section_card         (accordion body doesn't nest)
    #   * _wrap_as_lightbox             (accordion body doesn't nest)
    #   * _build_band_group             (accordion body doesn't nest)
    #   * _build_which_items_group      (Collection accordion subsumed it)
    #   * _build_what_to_do_group       (LaunchPad subsumed it)
    #   * _on_band_save_as_dc_clicked   (Save-as-DC surface retired)
    # The RecipeContainer + AccordionSection + StrictAccordionGroup +
    # LaunchPad quartet from spec/162 §4 handles every case these
    ...
    # spec/162 Round 2d.C — the Save-as-DC handler cluster
    # (_on_save_as_dc_clicked / _build_save_as_dc_payload /
    # _encode_rule_predicate_expr / _open_save_as_dc_dialog /
    # _append_operand_to_inventory) retired with the retirement of the
    # Save-as-Collection surface. Round 2d.F retires the Collection
    # operand type these helpers materialised into.
    ...
    def _on_overlay_field_toggled(self, _checked: bool = False) -> None:
        ...
        order matches the canonical :data:`OVERLAY_FIELDS` sequence
        (not click order — keeps the composition round-trip stable)."""
        from core import cut_overlay as _co
        keys = {
            key for key, chip in self._overlay_field_chips.items()
            if chip.isChecked()
        }
    ...
    # spec/162 Round 2d.C — the Load DC handler cluster
    # (_on_load_dc_clicked / _items_layer_has_state /
    # _confirm_replace_items / _apply_dc_to_items_layer) retired with
    # the Load-Collection sub-dialog. Round 2d.D retires the
    # ``dc_loader`` constructor kwarg they consumed.
