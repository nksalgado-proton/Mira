...
def dlg(qapp, tmp_path):
    ...
    # slide, which shifts the file indices by +1. These watchdog tests
    ...
    # Photo advance / watchdog behaviour doesn't depend on the title
    # slide being present.
    entries = [e for e in show_entries(
        gw, gw.cut("cut-s"), separators_on=False)
        if e[0] != "opener"]
    # Seed placeholder bytes for every file entry so the photo loader
    # doesn't spam Pillow warnings on _show_index calls; the video
    # branch never touches disk (the _StubPlayer handles bytes-in-tests).
