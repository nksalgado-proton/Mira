...
def test_session_files_join_source_facts(gw):
    ...
    assert by_rel["Edited Media/e2.jpg"].kind == "photo"
    assert by_rel["Edited Media/e2.jpg"].day_number == 1
    v = by_rel["Edited Media/v1.mp4"]
    ...
    assert by_rel["Edited Media/e3a.jpg"].source_item_id == "p3"
    assert by_rel["Edited Media/e3b.jpg"].source_item_id == "p3"
...
def test_default_picked_starts_full(gw):
    ...
    assert [f.export_relpath for f in s.picked_files()][0] == "Edited Media/e2.jpg"


def test_toggle_and_undo(gw):
    s = CutSession.from_draft(gw, _draft())
    assert s.toggle("Edited Media/e2.jpg") is True
    s.set_state("Edited Media/v1.mp4", True)
    assert s.picked_count() == 2
    assert s.undo() == "Edited Media/v1.mp4"
    assert s.picked_count() == 1
    assert s.undo() == "Edited Media/e2.jpg"
    assert s.picked_count() == 0
...
def test_totals_and_zone_follow_picks(gw):
    ...
    s.set_state("Edited Media/e2.jpg", True)      # day 1 photo
    s.set_state("Edited Media/e3a.jpg", True)     # day 2 photo
    t = s.totals()
    ...
    s.set_state("Edited Media/v1.mp4", True)      # +30 s of clip, no new day
...
def test_separators_off_zeroes_the_cards(gw):
    ...
    s.set_state("Edited Media/e2.jpg", True)
...
def test_days_grouping_in_show_order(gw):
    ...
    assert [(d, [f.export_relpath for f in fs]) for d, fs in groups] == [
        (1, ["Edited Media/e2.jpg"]),
        (2, ["Edited Media/e3a.jpg", "Edited Media/e3b.jpg",
             "Edited Media/v1.mp4"]),
    ]
...
def test_commit_creates_cut_with_membership(gw):
    s = CutSession.from_draft(gw, _draft())
    s.set_state("Edited Media/e3b.jpg", True)
    s.set_state("Edited Media/v1.mp4", True)
    cut = s.commit(gw)
    ...
    assert rels == ["Edited Media/e3b.jpg", "Edited Media/v1.mp4"]
...
def test_for_cut_with_draft_edits_settings_name_and_picks(gw):
    ...
    first.set_state("Edited Media/e2.jpg", True)
    cut = first.commit(gw)
    ...
    assert again.is_picked("Edited Media/e2.jpg")     # membership seeded
    assert any(f.export_relpath == "Edited Media/e1.jpg"
               for f in again.files)                  # widened pool visible
    again.set_state("Edited Media/v1.mp4", True)
    again.commit(gw)
    ...
    rels = [ln.export_relpath for ln in gw.cut_member_files(cut.id)]
    assert rels == ["Edited Media/e2.jpg", "Edited Media/v1.mp4"]
...
def test_reenter_seeds_from_membership_and_updates(gw):
    ...
    assert again.cut_id == cut.id
    assert again.is_picked("Edited Media/e2.jpg")
    assert not again.is_picked("Edited Media/e3a.jpg")

    again.set_state("Edited Media/e2.jpg", False)
    again.set_state("Edited Media/e3a.jpg", True)
    again.target_s = 300
    again.commit(gw)
    ...
    assert refreshed.target_s == 300
    rels = [ln.export_relpath for ln in gw.cut_member_files(cut.id)]
    assert rels == ["Edited Media/e3a.jpg"]
